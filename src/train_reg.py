...
def main(ticker, start, end, save):
    ...
    if df.empty:
        print('No data')
        return
    df = create_reg_target(df)
    df = add_basic_features(df)
    non_features = {'Date','next_close'}
    features = [c for c in df.columns if c not in non_features and df[c].dtype != 'O']
    print('Training reg with features:', features)
    train_xgb_regressor(df, features, save_name=save)
...
if __name__ == '__main__':
    ...
    p.add_argument('--save', default='xgb_reg_AAPL.joblib')
    args = p.parse_args()
